...
/**
 * The code-map lives in the gitignored cache, NOT in the tree.
 *
 * It was tracked until v0.62.0, and it is regenerated by `sync`, by the post-merge hook and by
 * autopilot's repair pass — so it changed under agents who never touched it. In one field report it
 * accounted for 40 of a repo's 88 commits, forced eight `git stash`es in a single day to allow a
 * branch switch, and rode into three unrelated feature PRs as a 105/75/25-line diff (2026-09-05 §5,
 * and §10 of the second report). No reader needs it committed: `doctor`, the briefing and the
 * bootstrap gate all read it locally, and it is deterministic — any machine can rebuild it with
 * `hivelore index code`. A repo whose own convention says "never commit a generated artifact"
 * should not ship one.
 */
export function codeMapPath(paths: HaivePaths): string {
  return path.join(paths.haiveDir, ".cache", CODE_MAP_FILE);
}

/** Where the code-map used to live, so an existing repo keeps working before it is migrated. */
export function legacyCodeMapPath(paths: HaivePaths): string {
  return path.join(paths.haiveDir, CODE_MAP_FILE);
}

/**
 * The code-map file actually in use: the cache copy, or the legacy tracked one on a repo that has
 * not been migrated yet. Every consumer that stats the file (freshness, doctor) must ask this, not
 * `codeMapPath`, or it will report "no map" for a repo whose map it just read.
 */
export function resolveCodeMapFile(paths: HaivePaths): string | null {
  const preferred = codeMapPath(paths);
  if (existsSync(preferred)) return preferred;
  const legacy = legacyCodeMapPath(paths);
...
export async function loadCodeMap(paths: HaivePaths): Promise<CodeMap | null> {
  // Read the cache location, falling back to the legacy tracked file so a repo that has not yet
  // re-run `sync`/`index code` keeps working with no visible change.
  const file = resolveCodeMapFile(paths);
  if (!file) return null;
...
export async function saveCodeMap(paths: HaivePaths, map: CodeMap): Promise<void> {
  ...
  if (current === payload && !legacyPresent) return;
  await mkdir(path.dirname(file), { recursive: true });
  await writeFile(file, payload, "utf8");
  // Migrate: remove the old tracked copy once the cache copy exists. This shows up as one
  // deletion in one commit — which is the whole point, versus a file that keeps re-appearing in
  // every agent's diff. Nothing is lost: the map is rebuilt from source on demand.
  if (legacyPresent) await rm(legacy, { force: true }).catch(() => {});
