...
/**
 * The `hivelore-sync` workflow, rendered for the repo's own integration branches.
 *
 * The push trigger used to be hardcoded to `[main, master]`. In a gitflow repo — which merges into
 * `develop` and promotes to `main` occasionally — that meant the sync job simply never ran: two
 * blocking sensors lived in `.ai/memories/` for six days without ever reaching `CLAUDE.md`, so
 * rules that failed commits were invisible to every agent supposed to obey them (field report
 * 2026-09-04 §4.3). Callers pass the branches they actually integrate on; the default list covers
 * both conventions.
 */
export function renderCiSyncWorkflow(integrationBranches: string[] = ["main", "master", "develop"]): string {
  return `name: hivelore-sync

on:
  push:
    branches: [${integrationBranches.join(", ")}]
...
jobs:
  ...
  sync-on-merge:
    ...
    steps:
      ...
      # 'sync' only touches .ai/. The breadcrumbs agents actually READ (CLAUDE.md, AGENTS.md, ...)
      # are regenerated by 'bridges sync', which was called nowhere - so a memory merged on this
      # branch never reached the file that delivers it (field report 2026-09-04 4.3).
      - name: refresh agent breadcrumbs (CLAUDE.md & co)
        run: hivelore bridges sync || true
  ...
  pr-memory-check:
    ...
    permissions:
      ...
      contents: read
    steps:
      # Enough history for the action to measure which files EVERY PR touches (a roadmap, a
      # changelog): those carry no information in a diff and are excluded from memory matching.
      # With a shallow checkout the measurement is skipped, never guessed.
      - uses: actions/checkout@v4
        with:
          fetch-depth: 100
  ...
  pr-eval-gate:
    ...
    steps:
      ...
      # Without a committed baseline this gate cannot fail — it recorded the same unfalsifiable
      # "100/100, no baseline, gate skipped" on twenty consecutive PRs, each paying a cold
      # 'npm install -g' of the CLI out of the account's Actions minutes (field report 2026-09-05
      # §6). Decide BEFORE installing anything, so an unconfigured gate costs seconds, not minutes.
      - name: is the regression gate configured?
        id: baseline
        run: |
          if [ -f .ai/eval/baseline.json ]; then
            echo "exists=true" >> "$GITHUB_OUTPUT"
          else
            echo "exists=false" >> "$GITHUB_OUTPUT"
            echo "::notice::No .ai/eval/baseline.json - harness regression gate not configured. Run 'hivelore eval --baseline' locally and commit it to enable this job."
          fi
      ...
      - name: harness quality trend
        if: always() && steps.baseline.outputs.exists == 'true'
...
/** Default rendering, for callers (and tests) that do not know the repo's branches. */
export const CI_WORKFLOW = renderCiSyncWorkflow();

/**
 * Which branches does this repo actually integrate on? Reads the local + remote branch list and
 * keeps the conventional integration names that EXIST here, so a gitflow repo gets `develop` in the
 * sync trigger and a trunk repo does not get a dead entry. Falls back to the generic list when git
 * is unavailable — a superset trigger is harmless (the job is idempotent), a missing one is not.
 */
export function detectIntegrationBranches(root: string): string[] {
  const candidates = ["main", "master", "develop", "development"];
  try {
    const out = spawnSync("git", ["branch", "--all", "--format=%(refname:short)"], { cwd: root, encoding: "utf8" });
    if (out.status !== 0 || !out.stdout) return ["main", "master", "develop"];
    const names = new Set(
      out.stdout.split("\n").map((line) => line.trim().replace(/^(?:remotes\/)?[^/]+\//, "")).filter(Boolean),
    );
    const found = candidates.filter((name) => names.has(name));
    return found.length > 0 ? found : ["main", "master", "develop"];
  } catch {
    ...
    .action(async (opts: {
      ...
      if (wantCi) {
        ...
        if (existsSync(ciPath)) {
          ...
          await writeFile(ciPath, renderCiSyncWorkflow(detectIntegrationBranches(root)), "utf8");
      ...
      await ensureGitignoreEntries(root, [
        ".ai/.cache/*",
        "!.ai/.cache/.gitignore",
        // The code-map is generated, regenerated by sync/post-merge/autopilot, and read only
        // locally — tracking it put a machine-written file in 40 of one repo's 88 commits and
        // forced a stash before every branch switch (field report 2026-09-05 §5).
        ".ai/code-map.json",
        ".ai/.runtime/*",
